...
export function useGithubProfile(username: string, pollMs = 600000) {
  ...
  const [loading, setLoading] = useState(!cachedVal); // Start with false if we have cached data
  const [error, setError] = useState<string | null>(null);
  useEffect(() => {
    let alive = true;
    const fetchOnce = () => {
      if (!cachedVal) setLoading(true); // Only show loading if no cache
      const run = async () => {
        try {
          if (getProfile) {
...
export function usePinnedRepos(username: string, pollMs = 600000) {
  ...
  const [loading, setLoading] = useState(!cachedVal);
  const [error, setError] = useState<string | null>(null);
  useEffect(() => {
    let alive = true;
    const run = async () => {
